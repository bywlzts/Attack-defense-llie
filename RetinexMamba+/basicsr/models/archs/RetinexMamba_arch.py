...
class Denoiser(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4],d_state= 16):
        ...
        self.moe_bottleneck = MoE_Block(nf=dim_level, mode='gaussian_only')
        ...
        for i in range(level):
            self.decoder_layers.append(nn.ModuleList([
                ...
                MoE_Block(nf=dim_level // 2, mode='mixed'),
    ...
    def forward(self, x, illu_fea, params_detach):
        """
        ...
        fea = self.moe_bottleneck(fea, params_detach)
        ...
        # 解码过程
        for i, (FeaUpSample, DAMoE, Fution, LeWinBlcok) in enumerate(self.decoder_layers):
            count+=1
            #print(f"第{count}次进行解码！")
            fea = FeaUpSample(fea)#进行反卷积，使特征向量分辨率变大，特征维度下降为原来的一半，此时illu_fea还没有变
            #print("")
            fea = DAMoE(fea, params_detach)
...
class RetinexMamba_Single_Stage(nn.Module):
    ...
    def forward(self, img, params_detach):
        """
        定义模型的前向传播路径。
        ...
        output_img = self.denoiser(input_img, illu_fea, params_detach)

        return output_img

class SFT_Layer(nn.Module):
    def __init__(self, nf=64, para_dim=2):
        super(SFT_Layer, self).__init__()

        # 将标量参数映射为特征向量
        self.param_mlp = nn.Sequential(
            nn.Linear(para_dim, nf // 4),
            nn.LeakyReLU(0.1),
            nn.Linear(nf // 4, nf),
            nn.Sigmoid()
        )

        # 条件卷积：融合 图像特征(空间信息) + 噪声参数(强度信息)
        ...
        # 生成 Scale 和 Shift
        self.to_gamma = nn.Conv2d(nf, nf, 1, 1, 0)
        self.to_beta = nn.Conv2d(nf, nf, 1, 1, 0)

        # 【关键】零初始化：确保初始状态下由 Identity Mapping 开始
        # 这样不会在训练初期破坏预训练特征
        nn.init.constant_(self.to_gamma.weight, 0)
        nn.init.constant_(self.to_gamma.bias, 0)
        nn.init.constant_(self.to_beta.weight, 0)
        nn.init.constant_(self.to_beta.bias, 0)

    def forward(self, x, params):
        b, c, h, w = x.size()

        p_feat = self.param_mlp(params)  # [B, nf]
        p_map = p_feat.view(b, c, 1, 1).expand(b, c, h, w)

        combined = torch.cat([x, p_map], dim=1)

        cond = self.cond_net(combined)
...
class MoE_Block(nn.Module):
    def __init__(self, nf=64, mode='mixed'):
        super(MoE_Block, self).__init__()
        self.mode = mode

        self.expert_gaussian = ResidualBlock_noBN(nf=nf)

        if self.mode == 'mixed':
            self.expert_poisson_conv1 = nn.Conv2d(nf, nf, 3, 1, 1)
            self.sft = SFT_Layer(nf=nf, para_dim=2)
            self.expert_poisson_conv2 = nn.Conv2d(nf, nf, 3, 1, 1)
    ...
    def forward(self, x, params):
        if self.mode == 'gaussian_only':
            return self.expert_gaussian(x)
        
        weights = self.gating_net(params)
        w_g = weights[:, 0].view(-1, 1, 1, 1)
        w_p = weights[:, 1].view(-1, 1, 1, 1)

        # Gaussian
        out_g = self.expert_gaussian(x)

        # Poisson
        res = self.expert_poisson_conv1(x)
        res = self.relu(res)
        res = self.sft(res, params) 
        res = self.expert_poisson_conv2(res)
        out_p = x + res 

        # Fusion
        return w_g * out_g + w_p * out_p


class IndependentNoisePredictor(nn.Module):
    def __init__(self, in_channels=3, param_dim=2):
        super(IndependentNoisePredictor, self).__init__()
...
class RetinexMamba(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1, 1, 1], d_state=16):
        ...
        self.noise_predictor = IndependentNoisePredictor(in_channels=3)
        self.body = nn.ModuleList(modules_body)
    
    def forward(self, x):
        pred_noise_params = self.noise_predictor(x)
        params_detach = pred_noise_params.detach()

        # 【修改点2】手动循环调用
        out = x
        for module in self.body:
            # 这里将 params_detach 传入每个阶段
            out = module(out, params_detach)

        return out, pred_noise_params
